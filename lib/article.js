...
async function waitForDomain(url, respectGate = true) {
  const domain = new URL(url).hostname;
  const previous = domainGates.get(domain) || 0;
  const wait = respectGate ? Math.max(0, previous - Date.now()) : 0;
  if (wait) await sleep(wait);
  const current = 2000 + Math.random() * 3000;
  domainGates.set(domain, Date.now() + current);
}
...
async function getBrowser() {
  ...
  if (!browserPromise) {
    browserPromise = process.env.VERCEL
      ? Promise.all([import('@sparticuz/chromium'), import('playwright-core')])
        .then(async ([{ default: chromium }, { chromium: playwrightChromium }]) => {
